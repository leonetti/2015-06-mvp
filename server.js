...
app.get('/location/:location', function (req, res) {
  ...
  yelp.search({term: "club", location: location, sort:2, limit:4,radius_filter:10000}, function(error, data) {
    if(error) res.end("Error");
    mapRequests("club",data.businesses);
  });

  yelp.search({term: "store", location: location, sort:2, limit:4,radius_filter:10000}, function(error, data) {
    if(error) res.end("Error");
